...
NHS.Scripts.CommunityResults.Map = function (mapParams) {
    ...
    this._communityResultsPage = mapParams.communityResultsPage;
    this._basicListingPage = mapParams.basicListingPage;
    this._basicCommPage = mapParams.basicCommPage;
    ...
    this._isBuilderMap = mapParams.isBuilderMap;
    this._brandId = mapParams.brandId;
    this._marketId = mapParams.marketId;
    this._brandName = mapParams.brandName;
    
    // Map size
    this._mapWidth = mapParams.Width;  
    this._mapHeight = mapParams.Height;
    ...
    if (typeof this._commResults != 'undefined') {
        this._log = this._commResults.get_log();
    } else {
        this._log = mapParams.logger;
    }
}
...
NHS.Scripts.CommunityResults.Map.prototype =
{
    ...
    show: function () {
        if (!this._isBeingShowed) {
            ...
            if (!this._isBuilderMap) {
                ...
                this._nhsMqMap.addControl(mqZoomControl, new MQA.MapCornerPlacement(MQA.MapCorner.TOP_RIGHT, new MQA.Size(10, 21)));
            }
            ...
            if (this._isBuilderMap)
                $jq('#nhs_CommResHideMap').hide();

            this._displayMapTip();

            $jq('#mqmaptilediv').css('z-index', '0');
    ...
    refresh: function () {
        ...
        if (this._searchParameters == null)
            radius = this._mqZoomLevel;
        else if (this._searchParameters.Radius >= 10)
            radius = 7;
    ...
    viewCommunityOnMap: function (e, cid, bid, name, cty, st, zip, img, brandimg, price, brand, matches, promoid, type, lat, lng, market, isBasicListing, isBasicComm) {
        this._doingViewonMap = true;


        if (isBasicListing == 'true')
            this.get_log().logEvent('CRBLMD', cid, bid);
        else if (isBasicComm == 'true')
            this.get_log().logEvent('CRBCMD', cid, bid);
        else
        ...
        if (!$jq('#nhs_CommResShowMap').is(".nhs_CommResShowMapOpen") && (typeof commResults != 'undefined'))
            commResults.showMap();
        ...
        if (!pointFound && this._nhsMqMap.getZoomLevel() >= 7) {
            commPoi = this._createCommPoi(cid, bid, name, cty, st, zip, img, brandimg, price, brand, matches, promoid, type, lat, lng, market, null, null, isBasicListing, isBasicComm);
        ...
        setTimeout((function () { this._doingViewonMap = false; }).bind(this), 1000);

        $jq('html,body').animate({ scrollTop: $jq("a[name='MapAnchor']").offset().top }, 'fast');
    ...
    browserResize: function () {
        ...
        if (this._isBuilderMap)
            var x = setTimeout("builderShowCase.get_map().browserResize()", 250);
        else
            var y = setTimeout("commResults.get_map().browserResize()", 250);
    },
    ...
    _mapMoveEnd: function (e) {
        if (this._updateMapFlag && !this._doingViewonMap) {
    ...
    _infoWindowOpen: function (e) {
        if (typeof commResults != 'undefined') {
    ...
    _updateMapPoints: function () {
        ...
        if (currentZoom < 4 && !this._isBuilderMap) {
            this._getStateMapPois(minLat, minLng, maxLat, maxLng);
        }
        else {
            if (currentZoom < 7 && (this._isBuilderMap && currentZoom < 6)) {
    ...
    _getCommMapPois: function (xmlSearchParams, minLat, minLng, maxLat, maxLng, maxResults) {
        // Builder Show Case Map
        if (this._isBuilderMap) {
            $jq.ajax({ url: this._mapCommsAction, data: { partnerId: this._partnerId, brandId: this._brandId, marketId: this._marketId }, success: this._onCommPoiSuccess.bind(this) });
        } else { // Comm Results Map 
    ...
    _getMarketMapPois: function (minLat, minLng, maxLat, maxLng) {
        // Builder Show Case Map
        if (this._isBuilderMap) {
            $jq.ajax({ url: this._mapMarketsAction, data: { partnerId: this._partnerId, brandId: this._brandId, minLat: this._mqMinLat, minLng: this._mqMinLng, maxLat: this._mqMaxLat, maxLng: this._mqMaxLng }, success: this._onMarketPoiSuccess.bind(this) });
        } else { // Comm Results Map 
    ...
    _onMarketPoiSuccess: function (result) {
        ...
        for (i = 0; i < result.length; i++) {
            ...
            if (this._isBuilderMap) {
                var communityResultsUrl = this._siteRoot + this._communityResultsPage + "/market-" + id + "/brandid-" + this._brandId;
                var marketName = title.substring(0, title.indexOf('(')).replace('<strong>', '');
                var commsCount = title.substring(title.indexOf('(')).replace('</strong>', '');
                contentHtml += "<h2><a href=\"" + communityResultsUrl + "\" >" + marketName + "</a></h2> <p>by " + this._brandName + "</p> <p>" + commsCount + "</p>";
                contentHtml += "</div>";
            } else {
                contentHtml += "<a href=\"javascript:void(0);\" onclick=\"if(typeof commResults != 'undefined') commResults.get_map().zoomToLatLng(" + lat + "," + lng + ",7);\">View Communities in " + content + "</a>";
                contentHtml += "</div>";
            }
    ...
    _onStatePoiSuccess: function (result) {
        ...
        for (i = 0; i < result.length; i++) {
            ...
            contentHtml += "<a href=\"javascript:void(0);\" onclick=\"if(typeof commResults != 'undefined') commResults.get_map().zoomToLatLng(" + lat + "," + lng + ",4);\">View Markets in " + stateName + "</a>";
            contentHtml += "</div>";
    ...
    _buildCommPois: function (commArray) {
        ...
        for (i = 0; i < commArrayLength; i++) {
            ...
            if (cAryLength == 19) {
                ...
                var isBasicComm = cAry[17] == "True";
                var isGreen = false; //cAry[16];
                var isHotHome = false; //cAry[17];

                var commPoint = this._createCommPoi(cid, bid, name, cty, st, zip, img, brandimg, price, brand, matches, promoid, type, lat, lng, market, isGreen, isHotHome, isBasicListing, isBasicComm);

            }
            else if (cAryLength > 19) {
                var commNumber = ((cAryLength - 1) / 18);
                ...
                var isBasicComm = false;
                var basicListingCount = 0;
                var basicCommCount = 0;
                ...
                for (j = 0; j < commNumber; j++) {
                    ...
                    var isBasicComm = cAry[(17 + k)] == "True";

                    if (isBasicListing) basicListingCount++;
                    if (isBasicComm) basicCommCount++;
                    ...
                    var basicCommUrl = this._siteRoot + this._basicCommPage + "/community" + cid + "/" + (name + "-" + cty + "-" + st + "-" + zip).replace(' ', '-').trim();
                    var communityUrl = this._siteRoot + this._communityPage + "/community-" + cid + "/builder-" + bid;
                    var comingSoonImg = this._resourceRoot + "GlobalResourcesMvc/Default/images/icons/icon2_coming_soon_overlay.png";

                    var detailsUrl = isBasicListing ? basicHomeUrl : (isBasicComm? basicCommUrl : communityUrl);
                    var basicPropertyCode = isBasicListing ? "CRBLMD" : "bcMapDetailBtn";
                    
                    var basicHomeLog = " onclick=\"if(typeof commResults != 'undefined') commResults.get_map().get_log().logBasicListingAndRedirect(event,'" + detailsUrl + "', '" + basicPropertyCode + "'," + cid + "); return false;\">";
                    var communityLog = " onclick=\"if(typeof commResults != 'undefined') commResults.get_map().get_log().logAndRedirect(event,'" + detailsUrl + "/" + "logevent-HOVNAME', 'HOVNAME', " + cid + "," + bid + "); return false;\">";

                    var detailsLog = isBasicListing || isBasicComm ? basicHomeLog : communityLog;
                    ...
                    if (type == "comingsoon") {
                        contentHtml.push("<img src=\"" + comingSoonImg + "\" class=\"nhs_ResultThumbComingSoonImg\" alt=\"Coming Soon\"/>");
                    }
                    ...
                    if (price != "$ - $" && price != '$2,147,483,647 - $') { // exclude max int value bc a bug in the search service
                    ...
                    if (!isBasicListing && !isBasicComm && !this._isBuilderMap && type != "comingsoon") {
                ...
                isBasicListing = basicListingCount == commNumber || basicCommCount == commNumber;
    ...
    _createCommPoi: function (cid, bid, name, cty, st, zip, img, brandimg, price, brand, matches, promoid, type, lat, lng, market, isGreen, isHotHome, isBasicListing, isBasicComm) {
        var typeImg = "";
        var typeAlt = "";
        ...
        if (price != "$ - $" && price != "" && price != '$2,147,483,647 - $') {
            contentHtml.push("<p class=\"nhs_ResultsPrice\">" + (!isBasicListing? "from " : ""));
        ...
        mqPoint = new MQA.Poi(mqPointLatLng, (isBasicListing || isBasicComm) ? this._nhsBLMapIcon : this._nhsMapIcon);
